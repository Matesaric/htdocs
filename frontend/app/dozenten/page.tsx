      ...
      <main>
        <div className="button-group">
          <div className="button-group-left">
            <button onClick={() => router.push("/dozenten/create")}>Neuer Dozent</button>
          ...
          <tbody>
            {!filteredData || filteredData.length === 0 ? (
              ...
              filteredData.map((p, idx) => (
                <tr key={String(p.id_dozent ?? p.id ?? idx)}>
                  <td>{p.vorname ?? "-"}</td>
                  <td>{p.nachname ?? "-"}</td>
                  ...
                  <td>{p.ort ?? "-"}</td>
                  <td>
                    <button onClick={() => router.push(`/dozenten/edit/${p.id_dozent ?? p.id}`)}>
                      Bearbeiten
                    </button>
                    <button onClick={() => router.push(`/dozenten/delete/${p.id_dozent ?? p.id}`)}>
                      Löschen
                    </button>
                  </td>
                </tr>
