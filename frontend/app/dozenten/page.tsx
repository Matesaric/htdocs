...
export default function DozentenPage() {
  ...
  const [deleteConfirm, setDeleteConfirm] = useState<{ item: Dozent | null; reason?: string } | null>(null);
  ...
  const confirmDelete = async () => {
    ...
    try {
      const resp = await fetch(`http://localhost/dozenten.php?id_dozent=${idStr}`, { method: "DELETE" });
      const text = await resp.text();
  ...
  return (
    <>
      ...
      {deleteConfirm?.item && (
        ...
        >
          <p>Möchten Sie diesen Dozenten wirklich löschen?</p>
          <div className="delete-info">
            ...
            <div className="delete-info-field">
              <strong>ID:</strong> {deleteConfirm.item.id_dozent ?? deleteConfirm.item.id}
            </div>
          </div>
        </ConfirmModal>
      )}
